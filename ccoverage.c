...
/*
 * Reads an input file, line-by-line, each line containing two space-separated numbers.
 * Each number pair is parsed and loaded into the locs array, appending the results starting at the position given by parameter first.
 */
int
load_locations (char *fn, float locs[][2], int first)
{
  ...
  while (fgets (line, MAXLINE, f) != NULL) {
    kwd = strtok (line, " "); // Get first space-separated token
    locs[nlocs][1] = atof (kwd); // parse first token into a float and save it into array
    kwd = strtok ((char *) NULL, " "); // Get second space-separated token
    locs[nlocs][0] = atof (kwd); // parse second token into a float and save it into the array
    nlocs += 1;
    if (nlocs >= MAXLOCS) {
...
/*
 * Do some calculations over each possible pair of different indexes of the locs array. O(n^2) where n = nlocs.
 */
void
