...
//            sum with MPI, using MPI_Reduce or using MPI_Receive into a buffer
//            and then summing at the root process both output the same result.
//            The error can be fixed by increasing the floating point precision
//            to double, though it also produces a different result, which I
//            assume is due to the error caused by using a float in the original
//            program.
